...
def read_energy(shciobj, state_id=None):
    ...
    if 'energy_total' in result:
        e =  next(iter(result['energy_total'][state_id].values()))['value'] 
        #e = result['energy_total'][state_id].values()[0]['value']

        shciobj.e_tot_t = next(iter(result['energy_total'][state_id].values()))['value'] 
        shciobj.e_var_t = result['energy_var'][state_id]
    else:
