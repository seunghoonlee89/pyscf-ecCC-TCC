...
def update_amps(cc, t1, t2, t1_t3c, t2_t3t4c, eris, thresh=None):
#def update_amps(cc, t1, t2, t3, t3p, t4, eris):
...
#    t1new +=0.5*einsum('kcld,iklacd->ia', eris_ovov, t3aaa)
#    t1new +=0.5*einsum('kcld,iklacd->ia', eris_ovov, t3aab)
#    t1new +=0.5*einsum('kcld,ilkadc->ia', eris_ovov, t3aab)
#    t1new +=0.5*einsum('kcld,klicda->ia', eris_ovov, t3aab)

#lsh: T3
#    t1new +=   einsum('kcld,klicda->ia', eris_ovov, t3p)
...
#    tmp2 += np.asarray(eris.ooov).conj().copy()
#    t2new-= einsum('kilc,lkjcab->ijab', tmp2, t3tmp)
#    tmp2  = einsum('kdlc,jd->kjlc', eris_ovov, t1)
#    tmp2 += np.asarray(eris.ooov).conj().copy()
#    t2new-= einsum('kjlc,likcab->ijab', tmp2, t3tmp)
#
#    tmp2  = -einsum('kcld,lb->kcbd', eris_ovov, t1)
#    tmp2 += eris_ovvv.conj().copy()
#    t2new+= einsum('kcbd,kijcad->ijab', tmp2, t3tmp)
#    tmp2  = -einsum('kcld,la->kcad', eris_ovov, t1)
#    tmp2 += eris_ovvv.conj().copy()
#    t2new+= einsum('kcad,kijcdb->ijab', tmp2, t3tmp)
#
#    tmp2  = 2*einsum('kcld,ld->kc', eris_ovov, t1)
#    tmp2 +=  -einsum('kdlc,ld->kc', eris_ovov, t1)
#    tmp2 += np.asarray(fov).conj().copy()
#    t2new+=  einsum('kc,kijcab->ijab', tmp2, t3tmp)
#
#    if thresh is None:
#        t2new +=0.5*einsum('kcld,klijcdab->ijab', eris_ovov, t4)
#    else:
##        # no truncation
##        eris_ovov = eris_ovov.reshape(nocc*nvir*nocc*nvir)
##        t4 = t4.transpose(0,4,1,5,2,3,6,7).reshape(nocc*nvir*nocc*nvir,nocc,nocc,nvir,nvir)
##        t2new +=0.5*einsum('s,sijab->ijab', eris_ovov, t4)
#
#        eris_ovov = eris_ovov.reshape(nocc*nvir*nocc*nvir)
#        t4 = t4.transpose(0,4,1,5,2,3,6,7).reshape(nocc*nvir*nocc*nvir,nocc,nocc,nvir,nvir)
#
#        ntrunc_eris = sum(1 for i in abs(eris_ovov) if i > thresh)
##        ntrunc_eris = eris_ovov.shape[0]                 # test
#        sort_index = np.argsort(-abs(eris_ovov))[0:ntrunc_eris]
#
#        eris_ovov_sort = eris_ovov[sort_index]
#        t4_sort = t4[sort_index,:,:,:,:]
#        print('thresh:',eris_ovov.shape)
#        print('eris_ovov shape:',eris_ovov.shape)
#        print('eris_ovov_sort shape:',eris_ovov_sort.shape)
#        print('t4 shape:',t4.shape)
#        print('t4_sort shape:',t4_sort.shape)
#        t2new +=0.5*einsum('s,sijab->ijab', eris_ovov_sort, t4_sort)

#lsh: T3, T4 contribution
#    tmp2  = einsum('kdlc,id->kilc', eris.ovov, t1)
#    tmp2 += np.asarray(eris.ooov).conj().transpose(0,1,2,3)
#    tmp = einsum('kilc,kljcab->ijab', tmp2, t3)
#    t2new += (tmp - tmp.transpose(1,0,2,3))
#
#    tmp2  = einsum('kcld,lb->kcbd', eris.ovov, -t1)
#    tmp2 += np.asarray(eris.ovvv).conj().transpose(0,1,2,3)
#    tmp = einsum('kcbd,kijcad->ijab', tmp2, t3)
#    t2new += (tmp - tmp.transpose(0,1,3,2))
#
#    tmp2  = 2*einsum('kcld,ld->kc', eris.ovov, t1)
#    tmp2 +=  -einsum('kdlc,ld->kc', eris.ovov, t1)
#    tmp2 += fov 
#    t2new +=  einsum('kc,kijcab->ijab', tmp2, t3)
#
#    t2new +=   einsum('kcld,klijcdab->ijab', eris.ovov, t4)

##lsh: T3 dbg
#    print ('dbg t1 eqn')
#    mo_e = eris.fock.diagonal().real
#    eia = mo_e[:nocc,None] - mo_e[None,nocc:]
#    t1new_dbg = einsum('ia,ia->ia', eia, t1)
#    for i in range (nocc):
#        for a in range (nvir):
#            print ('%d %d %20.7f %20.7f'%(i+1, a+nocc+1,t1new[i][a],t1new_dbg[i][a]))
#
##lsh: T4 dbg
#    print ('dbg t2 eqn')
#    eijab = lib.direct_sum('ia,jb->ijab',eia,eia)
#    t2new_dbg = einsum('ijab,ijab->ijab', eijab, t2)
#    for i in range (nocc):
#        for j in range (nocc):
#            for a in range (nvir):
#                for b in range (nvir):
#                    print ('%d %d %d %d %20.7f %20.7f'%(i+1, j+1, a+nocc+1, b+nocc+1,t2new[i][j][a][b],t2new_dbg[i][j][a][b]))
...
#    print('min eia',np.amin(abs(eia.reshape((-1)))))
#    print('min eijab',np.amin(abs(eijab.reshape((-1)))))
...
class ecRCCSD(ccsd.CCSD):
    ...
    def kernel(self, coeff, t1=None, t2=None, t3=None, t3p=None, t4=None, eris=None, mbpt2=False, cc2=False, ci2cc=None, thresh=None, stochastic=False):
        return self.ccsd(coeff, t1, t2, t3, t3p, t4, eris, mbpt2, cc2, ci2cc, thresh=thresh, stochastic=stochastic)
    def ccsd(self, coeff, t1=None, t2=None, t3=None, t3p=None, t4=None, eris=None, mbpt2=False, cc2=False, ci2cc=None, thresh=None, stochastic=False):
        '''Ground-state CCSD.
        ...
        if mbpt2:
            ...
            self.converged, self.e_corr, self.t1, self.t2 = \
                    ccsd.ec_kernel_slow(self, coeff, eris, t1, t2, t3, t3p, t4, ci2cc, max_cycle=self.max_cycle,
                                tol=self.conv_tol, tolnormt=self.conv_tol_normt,
                                verbose=self.verbose, thresh=thresh, stochastic=stochastic)
#            if self.converged:
...
#lsh test
    def correlation_t4_2e_test(self, t4, eris):
        nocc = self.nocc
        nvir = self.nmo - nocc
    
        eris_ovov = np.asarray(eris.ovov)
        p_ref     = 0.5*einsum('kcld,klijcdab->ijab', eris_ovov, t4).reshape(nocc*nvir*nocc*nvir)
        p_prime   = 0.5*einsum('kcld,klijcdab->kcldijab', eris_ovov, t4).reshape(nocc*nvir*nocc*nvir,nocc*nvir*nocc*nvir)
    
        eris_ovov = eris_ovov.reshape(nocc*nvir*nocc*nvir)
        sort_index = np.argsort(-abs(eris_ovov))
        eris_ovov  = eris_ovov[sort_index]
        p_prime    = p_prime[sort_index,:]
#        print("eris_ovov")
#        print(eris_ovov)
#        print("p_prime")
#        print(p_prime)
   
        accum = np.zeros((nocc*nvir*nocc*nvir)) 
        errort= np.zeros((nocc*nvir*nocc*nvir)) 
        for f1 in range(nocc*nvir*nocc*nvir):
            for l in range(nocc*nvir*nocc*nvir):
                accum[l] += p_prime[f1,l]
            errort[f1] = np.linalg.norm (accum - p_ref) 
            print(f1, eris_ovov[f1], np.linalg.norm (accum - p_ref) )

        from matplotlib import pyplot as plt
        x = [i for i in range(nocc*nvir*nocc*nvir)]
        plt.plot(x,errort) 
        plt.show()
