...
'''
Tailored UCCSD with spatial integrals
'''
...
def update_amps(cc, t1, t2, eris):
    ...
    # Tailored UCCSD
    t1af, t1bf = cc.t1f
    t2aaf, t2abf, t2bbf = cc.t2f

    u1a = t1a * t1af + u1a * (1-t1af)
    u1b = t1b * t1bf + u1b * (1-t1bf)
    u2aa= t2aa * t2aaf + u2aa * (1-t2aaf)
    u2ab= t2ab * t2abf + u2ab * (1-t2abf)
    u2bb= t2bb * t2bbf + u2bb * (1-t2bbf)
...
#       orbitals, second list is for beta orbitals
    ...
    def kernel(self, mc, nocc_cas=None, nvir_cas=None, nocc_corr=None, nvir_corr=None, ext_source="FCI", t1=None, t2=None, eris=None, mbpt2=False, coeff=None):
        if nocc_cas  == None:
            nocca_cas  = mc.nelecas[0]
            noccb_cas  = mc.nelecas[1]
            nocc_cas = (nocca_cas, noccb_cas)
        if nvir_cas  == None:
            nvira_cas  = mc.ncas - nocca_cas
            nvirb_cas  = mc.ncas - noccb_cas
            nvir_cas   = (nvira_cas, nvirb_cas)
        if nocc_corr == None:
            nocca_corr = (self.mol.nelectron + self.mol.spin)//2
            noccb_corr = (self.mol.nelectron - self.mol.spin)//2
            nocc_corr  = (nocca_corr, noccb_corr)
        if nvir_corr == None:
            nvira_corr = self.mo_coeff[0].shape[1] - nocca_corr
            nvirb_corr = self.mo_coeff[0].shape[1] - noccb_corr
            nvir_corr  = (nvira_corr, nvirb_corr)
        if coeff == None:
            import os
            ...
            if ext_source == "FCI" or ext_source == "fci":
                ...
                idxa = fci_index_nomem(nocca_cas, nvira_cas) 
                idxb = fci_index_nomem(noccb_cas, nvirb_cas) 
                idx  = (idxa, idxb)
                from pyscf.cc.fci_coeff import ufci_coeff 
                self.coeff = ufci_coeff(mc.ci, idx)

            elif ext_source == "SHCI" or ext_source == "shci":
        ...
        self.coeff.nocc_iact= (nocca_corr-nocca_cas, noccb_corr-noccb_cas)
        self.coeff.nvir_iact= (nvira_corr-nvira_cas, nvirb_corr-nvirb_cas) 
...
#        print(nvir_cas)
    ...
    def ccsd(self, t1=None, t2=None, eris=None, mbpt2=False):
        ...
        return ccsd.CCSD.ccsd(self, t1, t2, eris, TCCSD_tmp=True, coeff=self.coeff)
