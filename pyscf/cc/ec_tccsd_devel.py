...
def update_amps(cc, t1, t2, t1_t3c, t2_t3t4c, t1f, t2f, eris):
    ...
    # Tailored CCSD
    t1new = t1 * t1f + t1new * (1-t1f)
    t2new = t2 * t2f + t2new * (1-t2f)
...
def energy(cc, t1=None, t2=None, eris=None):
    ...
    err = 1e-10
    Write_input_for_MPS_to_CI(nelectron, symmetry, err, gaopt)
    ...
    os.system('cp CIcoeff_dmrg.out CIcoeff_shci.out')

class ecRTCCSD(ccsd.CCSD):
    ...
    def kernel(self, nocc_corr, nvir_corr, nocc_cas, nvir_cas, ext_source="SHCI", t1=None, t2=None, eris=None, numzero=1E-12, coeff=None):
        ...
        return self.ccsd(self.coeff, eris, t1, t2, numzero)
    def ccsd(self, coeff, eris=None, t1=None, t2=None, numzero=1E-12):
        '''Ground-state externally corrected CCSD.
        '''
        if eris is None:
            eris = self.ao2mo(self.mo_coeff)

        return ccsd.CCSD.ccsd(self, t1=t1, t2=t2, eris=eris, ecTCCSD_devel=True, coeff=coeff, numzero=numzero)
...
#?        assert(t2.dtype == np.double)
        return ccsd.CCSD.ccsd_t(self, t1, t2, eris, ecTCCSD_devel=True, coeff=self.coeff)

    def rccsd_t(self, t1=None, t2=None, eris=None):
        return ccsd.CCSD.rccsd_t(self, t1, t2, eris, ecTCCSD_devel=True, coeff=self.coeff)
