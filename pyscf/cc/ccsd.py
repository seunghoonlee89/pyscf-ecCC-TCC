...
# t1: ia
# t2: ijab
#def ec_kernel(mycc, eris=None, t1=None, t2=None, t3=None, t3p=None, t4=None, max_cycle=50, tol=1e-8,
#def ec_kernel(mycc, eris=None, t1=None, t2=None, t3aaa=None, t3aab=None, t4=None, max_cycle=50, tol=1e-8,
def ec_kernel_devel(mycc, coeff, eris=None, t1=None, t2=None, t3aaa=None, t3aab=None, t4=None, ci2cc=None, max_cycle=50, tol=1e-8,
           tolnormt=1e-6, verbose=None, thresh=None, stochastic=False, numzero=1E-5):
    log = logger.new_logger(mycc, verbose)
    if eris is None:
        eris = mycc.ao2mo(mycc.mo_coeff)
    ...
    eris_ovov = np.asarray(eris.ovvo).transpose(0,1,3,2).copy()
    eris_ovvv = np.asarray(eris.ovvv)
    if not mycc.t4contraction and mycc.restart <= 0 and not mycc.onthefly:  
        # thresh
        if thresh is not None:
            eris_ovov_sort = eris_ovov.copy().reshape(nocc*nvir*nocc*nvir)
            ntrunc_eris = sum(1 for i in abs(eris_ovov_sort) if i > thresh)
            sort_index = np.argsort(-abs(eris_ovov_sort))[0:ntrunc_eris]
            eris_ovov_sort = eris_ovov_sort[sort_index]
            print('n_tot, n_trunc', nocc*nvir*nocc*nvir, ntrunc_eris)
        elif stochastic:
            cont1 =einsum('kcld->klc', abs(eris_ovov))
            cont2 =einsum('klc->kl', abs(cont1))
            cont3 =einsum('kl->k', abs(cont2))
            cont4 =einsum('k->', abs(cont3))
            p_d_klc = einsum('kcld,klc->klcd', abs(eris_ovov), 1/cont1)
            p_d_klc = np.cumsum(p_d_klc, axis=3)  
            p_c_kl  = einsum('klc,kl->klc', cont1, 1/cont2)
            p_c_kl  = np.cumsum(p_c_kl, axis=2) 
            p_l_k   = einsum('kl,k->kl', cont2, 1/cont3)
            p_l_k   = np.cumsum(p_l_k, axis=1)  
            p_k     = cont3 / cont4 
            p_k     = np.cumsum(p_k)  
            ...
            if dbg:
                ...
                for k in range (nocc):
                    for l in range (nocc):
                        print ('l|k=',k,l, p_l_k[k][l])
        
                for k in range (nocc):
                    for l in range (nocc):
                        for c in range (nvir):
                            print ('c|kl=',k,l,c, p_c_kl[k][l][c])
        
                for k in range (nocc):
                    for l in range (nocc):
                        for c in range (nvir):
                            for d in range (nvir):
                                print ('d|klc=',k,l,c,d, p_d_klc[k][l][c][d])

    # gen index for ci coeff
#        if not cWrapper: idx = fci_index(nocc, nvir) 
#        else: idx = fci_index_c(nocc, nvir) 
...
#        idx.get_Q()
        ...
        # gen CI coeff # 
        ################
        if not coeff.flagS: coeff.get_S(0)
        if not coeff.flagD: coeff.get_D(0)
        if not coeff.flagT: coeff.get_T(0)
        if thresh is None:
            if not coeff.flagQ: coeff.get_Q(0)
        else:
            if not coeff.flagQ: coeff.get_Q_thresh(sort_index, coeff.idx)
...
#        norm += numpy.sum(numpy.square ( coeff.D_bb ))
        ...
        norm += 2.0*numpy.sum(numpy.square ( coeff.T_aaa ))
        norm += 2.0*numpy.sum(numpy.square ( coeff.T_aab ))
#        norm += numpy.sum(numpy.square ( coeff.T_abb ))
#        norm += numpy.sum(numpy.square ( coeff.T_bbb ))
        print (' 0SDT (T) =', norm,'(', norm-norm0SD,')')
        norm0SDT = norm
        norm += 2.0*numpy.sum(numpy.square ( coeff.Q_aaab ))
        norm += numpy.sum(numpy.square ( coeff.Q_aabb ))
#        norm += numpy.sum(numpy.square ( coeff.Q_abbb ))
        print ('0SDTQ (Q) =', norm,'(', norm-norm0SDT,')')
    
#        log.info('max_memory %d MB (current use %d MB after gen CI coeff)',
#                 mycc.max_memory, lib.current_memory()[0])
#        cput1 = log.timer('Gen CI coeff', *cput1)
        ...
        if not cWrapper: mycc.ci2cc = fci_to_cc(nocc, nvir, coeff.idx, coeff.Ref)
        else: mycc.ci2cc = fci_to_cc_c(nocc, nvir, coeff.idx, coeff.Ref)
    
        mycc.ci2cc.c1_to_t1(coeff.S_a.copy())
        mycc.ci2cc.c2_to_t2(coeff.D_aa.copy(), coeff.D_ab.copy())
        if not mycc.extract_thresh: mycc.ci2cc.c3_to_t3(coeff.T_aaa.copy(), coeff.T_aab.copy(), numzero=numzero) 
        else: mycc.ci2cc.c3_to_t3_thresh(coeff.T_aaa.copy(), coeff.T_aab.copy(), numzero=numzero)

        
        if thresh is not None:
            # have to change
            mycc.ci2cc.c4_to_t4(coeff.Q_aaab.copy(), coeff.Q_aabb.copy(), numzero=numzero)
            t4_sort = mycc.ci2cc.t4.copy().transpose(0,4,1,5,2,3,6,7).reshape(nocc*nvir*nocc*nvir,nocc,nocc,nvir,nvir)
            t4_sort = t4_sort[sort_index,:,:,:,:]
        else:
            if not mycc.extract_thresh: mycc.ci2cc.c4_to_t4(coeff.Q_aaab.copy(), coeff.Q_aabb.copy(), numzero=numzero)
            else: mycc.ci2cc.c4_to_t4_thresh(coeff.Q_aaab.copy(), coeff.Q_aabb.copy(), numzero=numzero)


        cput1 = log.timer('Gen t amplitude c', *cput1)

        if dbg:
            norm1 = numpy.sum(numpy.square (mycc.ci2cc.t3aaa))
            norm2 = numpy.sum(numpy.square (mycc.ci2cc.t3aab))
            print ('t3aaa norm =', norm1)
            print ('t3aab norm =', norm2)
        ...
        if dbg:
            norm1 = numpy.sum(numpy.square (ci2cc.t4))
            print ('t4 norm =', norm1)
    
        t2_t4c = numpy.zeros((nocc,nocc,nvir,nvir), dtype=numpy.float64)
        if thresh is not None:
            t2_t4c =0.5*einsum('s,sijab->ijab', eris_ovov_sort, t4_sort)
        elif stochastic:
            num_samp=20000
            mean = np.zeros((nocc,nocc,nvir,nvir)) 
            for it in range(num_samp):
                rand = np.random.uniform(0,1,1)
    
                k = [t for t in range(len(p_k)) if p_k[t] > rand][0]
                rand = np.random.uniform(0,1,1)
                l = [t for t in range(len(p_l_k[k])) if p_l_k[k][t] > rand][0] 
                rand = np.random.uniform(0,1,1)
                c = [t for t in range(len(p_c_kl[k][l])) if p_c_kl[k][l][t] > rand][0] 
                rand = np.random.uniform(0,1,1)
                d = [t for t in range(len(p_d_klc[k][l][c])) if p_d_klc[k][l][c][t] > rand][0]
                for i in range(nocc):
                    for j in range(nocc):
                        for a in range(nvir):
                            for b in range(nvir):
                                mean[i][j][a][b] += np.sign(eris_ovov[k][c][l][d]) * ci2cc.t4[k][l][i][j][c][d][a][b] 
            mean /= num_samp
            t2_t4c =0.5*mean*cont4
        else:
            #print('check t4', ci2cc.t4[3][3][2][1][0][0][4][4],ci2cc.t4[3][3][1][2][0][0][4][4])
            t2_t4c =0.5*einsum('kcld,klijcdab->ijab', eris_ovov, ci2cc.t4)
    #        p_d_klc = einsum('kcld,kcl->kcld', eris_ovov, cont1)
    #        p_c_kl  = einsum('kcl,kl->kcl', cont1, cont2)
    #        p_l_k   = einsum('kl,k->kl', cont2, cont3)
    #        p_k     = cont3 / cont4 

    #    if dbg:
#        log.info('max_memory %d MB (current use %d MB after contraction)',
#                 mycc.max_memory, lib.current_memory()[0])
#        cput1 = log.timer('t3 t4 contribution', *cput1)

    elif mycc.t4contraction and mycc.restart <= 0 and not mycc.onthefly:  
        #################
        # read CI coeff #
        #################
        print('=== start reading SDT CI coeff ===')
        if (not coeff.flagS) or (not coeff.flagD) or (not coeff.flagT): 
            coeff.get_SDT(mycc.nc)
        #lsh test
        norm = numpy.square( coeff.Ref[0] )
        print ('        0 =', norm )
        norm0 = norm
        ...
        assert not coeff.interm_norm_S and not coeff.interm_norm_D \
           and not coeff.interm_norm_T and not coeff.interm_norm_Q
        coeff.interm_norm(Q=False)

        log.info('max_memory %d MB (current use %d MB)',
                 mycc.max_memory, lib.current_memory()[0])
        cput1 = log.timer('reading SDT CI coeff                    ', *cput1)
        print('=== END reading SDT CI coeff ===')

        ########################
        # div files for c3, c4 #
        ########################
        print('=== start dividing c3 and c4 files ===')
        num_threads = lib.num_threads()
        if num_threads > 1:
...
#            coeff.del_df() 
        log.info('max_memory %d MB (current use %d MB)',
                 mycc.max_memory, lib.current_memory()[0])
        cput1 = log.timer('dividing c3 and c4 files                ', *cput1)
        print('=== END dividing c3 and c4 files ===')
   
        ########################
        # extract and contract #
        ########################
        # t4 for t2 amplitude eqn
        print('=== start extracting and contracting t4 for t2 eqn ===')
        if not cWrapper: mycc.ci2cc = fci_to_cc(nocc, nvir, coeff.idx, coeff.Ref)
        else: mycc.ci2cc = fci_to_cc_c(nocc, nvir, coeff.idx, coeff.Ref)
        ci2cc = mycc.ci2cc
        ci2cc.c1_to_t1(coeff.S_a.copy())
        ...
        log.info('max_memory %d MB (current use %d MB)',
                 mycc.max_memory, lib.current_memory()[0])
        cput1 = log.timer('extracting and contracting t4 for t2 eqn', *cput1)
        print('=== END extracting and contracting t4 for t2 eqn ===')

        # t3 for t1 amplitude eqn
        print('=== start extracting and contracting t3 for t1 eqn ===')
        t1_t3c =0.5*einsum('kcld,iklacd->ia', eris_ovov, ci2cc.t3aaa)
        t1_t3c+=0.5*einsum('kcld,iklacd->ia', eris_ovov, ci2cc.t3aab)
        t1_t3c+=0.5*einsum('kcld,ilkadc->ia', eris_ovov, ci2cc.t3aab)
        t1_t3c+=0.5*einsum('kcld,klicda->ia', eris_ovov, ci2cc.t3aab)
        ...
        log.info('max_memory %d MB (current use %d MB)',
                 mycc.max_memory, lib.current_memory()[0])
        cput1 = log.timer('extracting and contracting t3 for t1 eqn', *cput1)
        print('=== END extracting and contracting t3 for t1 eqn ===')
   
    elif mycc.t4contraction and mycc.restart <= 0 and mycc.onthefly:  
        #################
        # read CI coeff #
        #################
        print('=== start reading SDT CI coeff ===')
        if (not coeff.flagS) or (not coeff.flagD) or (not coeff.flagT): 
            coeff.get_SDT(mycc.nc) #TODO: to remove saving c3 but on-the-fly reading c3
...
#            coeff.del_df() 
        ...
        t2_t4c = numpy.zeros((nocc,nocc,nvir,nvir), dtype=numpy.float64)
        if num_threads > 1:
            coeff.get_t2t4c_omp_otf(t2_t4c, eris_ovov, ci2cc, numzero, mycc.nc, norm)
        else:
            raise NotImplementedError
            #coeff.get_t2t4c_otf(t2_t4c, eris_ovov, ci2cc, numzero, mycc.nc, norm)
        ...
        t1_t3c = numpy.zeros((nocc,nvir), dtype=numpy.float64)
        if num_threads > 1:
            coeff.get_t1t3c_omp(t1_t3c, eris_ovov, ci2cc, numzero, mycc.nc) 
        else:
            coeff.get_t1t3c(t1_t3c, eris_ovov, ci2cc, numzero, mycc.nc) 
...
#    adiis = None
    ...
    if t3_0_t1_0_approx:
        # t3 for t2 amplitude eqn with extracted t1 
        print('=== start extracting and contracting t3 for t2 eqn ===')
        if not mycc.onthefly:
            t2_t3t4c = t2_t4c.copy()
            t3tmp = ci2cc.t3aab.copy() + ci2cc.t3aab.transpose(0,2,1,3,5,4).copy() 
            tmp2  = einsum('kdlc,id->kilc', eris_ovov, ci2cc.t1)
            ...
            tmp3 +=  -einsum('kdlc,ld->kc', eris_ovov, ci2cc.t1)
            tmp3 += np.asarray(fov).conj().copy()
            coeff.get_t2t3c_omp(t2_t3t4c, tmp1, tmp2, tmp3, ci2cc, numzero, mycc.nc)

        log.info('max_memory %d MB (current use %d MB)',
                 mycc.max_memory, lib.current_memory()[0])
        cput1 = log.timer('extracting and contracting t3 for t2 eqn', *cput1)
        print('=== END extracting and contracting t3 for t2 eqn ===')
    ...
    for istep in range(max_cycle):
        ...
        t1new, t2new = mycc.update_amps(t1, t2, t1_t3c, t2_t3t4c, eris)
        if mycc.scale_correct:
            assert mycc.restart <= 0
            for i in range(nocc):
                for j in range(nocc):
                    for a in range(nvir):
                        for b in range(nvir):
                            l = numpy.float64(a+b+2*nocc-i-j)/numpy.float64(2*(nocc+nvir))
                            t2new[i][j][a][b] = l * t2new[i][j][a][b] \
                                              + (1-l) * ci2cc.t2ab[i][j][a][b]
...
#lsh test
#    conv=False; eccsd=0.0;
    return conv, eccsd, t1, t2

def ect_kernel(mycc, coeff, eris=None, t1=None, t2=None, t3aaa=None, t3aab=None, t4=None, ci2cc=None, max_cycle=50, tol=1e-8,
           tolnormt=1e-6, verbose=None, thresh=None, stochastic=False, numzero=1E-5):
    log = logger.new_logger(mycc, verbose)
    if eris is None:
        eris = mycc.ao2mo(mycc.mo_coeff)
    ...
    eris_ovov = np.asarray(eris.ovvo).transpose(0,1,3,2).copy()
    eris_ovvv = np.asarray(eris.ovvv)
    if not mycc.t4contraction and mycc.restart <= 0:  
        raise NotImplementedError
    # gen index for ci coeff
#        if not cWrapper: idx = fci_index(nocc, nvir) 
#        else: idx = fci_index_c(nocc, nvir) 
...
#        cput1 = log.timer('t3 t4 contribution', *cput1)
    ...
    elif mycc.t4contraction and mycc.restart <= 0:  
        # gen CI coeff
        assert (not coeff.flagS) and (not coeff.flagD) \
           and (not coeff.flagT) and (not coeff.flagQ) 
        #TODO: until now, nc = 0, change it
        print('=== start extracting SDT CI coeff ===')
        coeff.get_SDT(mycc.nc)
        print('=== END extracting SDT CI coeff ===')
...
#        cput1 = log.timer('Gen CI coeff', *cput1)
        ...
        # extract t amplitude 
        print('=== start making ci to t obj ===')
        if not cWrapper: mycc.ci2cc = fci_to_cc(nocc, nvir, coeff.idx, coeff.Ref)
        else: mycc.ci2cc = fci_to_cc_c(nocc, nvir, coeff.idx, coeff.Ref)
        ci2cc = mycc.ci2cc
        print('=== END making ci to t obj ===')

        ###################### 
        # T3, T4 contraction # 
        ###################### 
        t2_t4c = numpy.zeros((nocc,nocc,nvir,nvir), dtype=numpy.float64)
        print('=== start extracting SDT t amplitude and contraccting Q t amplitude ===')
        #coeff.get_t2t4(t2_t4c, eris_ovov, ci2cc, numzero, 0)
        coeff.get_t2t4c_ecT(t2_t4c, eris_ovov, ci2cc, numzero, mycc.nc)
        print('=== end extracting SDT t amplitude and contraccting Q t amplitude ===')
#
        t1_t3c =0.5*einsum('kcld,iklacd->ia', eris_ovov, ci2cc.t3aaa)
        t1_t3c+=0.5*einsum('kcld,iklacd->ia', eris_ovov, ci2cc.t3aab)
        t1_t3c+=0.5*einsum('kcld,ilkadc->ia', eris_ovov, ci2cc.t3aab)
        ...
        cput1 = log.timer('Read t amplitude c', *cput1)
        log.info('max_memory %d MB (current use %d MB after gen t amplitude)',
                 mycc.max_memory, lib.current_memory()[0])
...
#    adiis = None
    ...
    t1 = ci2cc.t1.copy() * t1f.copy()   + t1 * (1-t1f.copy())
    t2 = ci2cc.t2ab.copy() * t2f.copy() + t2 * (1-t2f.copy())

    ectccsd = mycc.energy(t1, t2, eris)
    log.info('Init E_corr(ecTCCSD) = %.15g', ectccsd)

#lsh test
    if t3_0_t1_0_approx:
        t3tmp = ci2cc.t3aab.copy() + ci2cc.t3aab.transpose(0,2,1,3,5,4).copy() 
        tmp2  = einsum('kdlc,id->kilc', eris_ovov, ci2cc.t1 * (1-t1f.copy()))
        tmp2 += np.asarray(eris_ooov).conj().copy()
        t2_t3t4c  = t2_t4c -einsum('kilc,lkjcab->ijab', tmp2, t3tmp)
        tmp2  = einsum('kdlc,jd->kjlc', eris_ovov, ci2cc.t1 * (1-t1f.copy()))
        tmp2 += np.asarray(eris_ooov).conj().copy()
        t2_t3t4c -= einsum('kjlc,likcab->ijab', tmp2, t3tmp)
    
        tmp2  = -einsum('kcld,lb->kcbd', eris_ovov, ci2cc.t1 * (1-t1f.copy()))
        tmp2 += eris_ovvv.conj().copy()
        t2_t3t4c+= einsum('kcbd,kijcad->ijab', tmp2, t3tmp)
        tmp2  = -einsum('kcld,la->kcad', eris_ovov, ci2cc.t1 * (1-t1f.copy()))
        tmp2 += eris_ovvv.conj().copy()
        t2_t3t4c+= einsum('kcad,kijcdb->ijab', tmp2, t3tmp)
    
        tmp2  = 2*einsum('kcld,ld->kc', eris_ovov, ci2cc.t1 * (1-t1f.copy()))
        tmp2 +=  -einsum('kdlc,ld->kc', eris_ovov, ci2cc.t1 * (1-t1f.copy()))
        tmp2 += np.asarray(fov).conj().copy()
        t2_t3t4c+=  einsum('kc,kijcab->ijab', tmp2, t3tmp)
    ...
    for istep in range(max_cycle):
        ...
        t1, t2 = t1new, t2new
        t1new = t2new = None
        t1, t2 = mycc.run_diis(t1, t2, istep, normt, ectccsd-eold, adiis)
        eold, ectccsd = ectccsd, mycc.energy(t1, t2, eris)
        log.info('cycle = %d  E_corr(ecTCCSD) = %.15g  dE = %.9g  norm(t1,t2) = %.6g',
                 istep+1, ectccsd, ectccsd - eold, normt)
...
def tcc_kernel(mycc, coeff, eris=None, t1=None, t2=None, t3aaa=None, t3aab=None, t4=None, ci2cc=None, max_cycle=50, tol=1e-8,
           tolnormt=1e-6, verbose=None, stochastic=False, numzero=1E-5):
    log = logger.new_logger(mycc, verbose)
    if eris is None:
        eris = mycc.ao2mo(mycc.mo_coeff)
...
#   idx.get_Q()
    ...
    # gen CI coeff # 
    ################
    if not coeff.flagS or not coeff.flagD: coeff.get_SD(mycc.nc)
...
#   cput1 = log.timer('Gen CI coeff', *cput1)
    ...
    #t1f, t2f = coeff.tcc_tcas_idx()
    t1f, t2f = coeff.ectcc_tcas_idx()
#    t1 = mycc.ci2cc.t1.copy() * t1f.copy()   + t1 * (1-t1f.copy())
#    t2 = mycc.ci2cc.t2ab.copy() * t2f.copy() + t2 * (1-t2f.copy())
...
#        adiis = None
    ...
    for istep in range(max_cycle):
        ...
        t1new, t2new = mycc.update_amps(t1, t2, t1f, t2f, eris)
        ...
        t1, t2 = t1new, t2new
        t1new = t2new = None
        t1, t2 = mycc.run_diis(t1, t2, istep, normt, tccsd-eold, adiis)
        eold, tccsd = tccsd, mycc.energy(t1, t2, eris)
...
#lsh test
#    conv=False; tccsd=0.0;
    return conv, tccsd, t1, t2

#lsh
def tcc_kernel_new(mycc, coeff, eris=None, t1=None, t2=None, max_cycle=50, tol=1e-8,
           tolnormt=1e-6, verbose=None, numzero=1E-5):
    log = logger.new_logger(mycc, verbose)
    if eris is None:
        eris = mycc.ao2mo(mycc.mo_coeff)
    ...
    if t1 is None or t2 is None:
        ...
        log.info('max_memory %d MB (current use %d MB)',
                 mycc.max_memory, lib.current_memory()[0])
        cput1 = log.timer('extracting c1 and c2', *cput1)
        ...
        nvir_tcas = coeff.nvir_tcas

        if isinstance(nocc_tcas, int) and isinstance(nvir_tcas, int):
            nocc_iact = nocc - nocc_tcas 
            for i in range(nocc_tcas): 
                for a in range(nvir_tcas): 
                    mycc.t1f[i+nocc_iact][a] = 1.0
            for i in range(nocc_tcas): 
                for j in range(nocc_tcas): 
                    for a in range(nvir_tcas): 
                        for b in range(nvir_tcas): 
                            mycc.t2f[i+nocc_iact][j+nocc_iact][a][b] = 1.0
        elif isinstance(nocc_tcas, list) and isinstance(nvir_tcas, list):
            for i in nocc_tcas: 
                for a in nvir_tcas: 
                    mycc.t1f[i][a] = 1.0
            for i in nocc_tcas: 
...
#def ec_kernel(mycc, eris=None, t1=None, t2=None, t3=None, t3p=None, t4=None, max_cycle=50, tol=1e-8,
#def ec_kernel(mycc, eris=None, t1=None, t2=None, t3aaa=None, t3aab=None, t4=None, max_cycle=50, tol=1e-8,
def ec_kernel_slow(mycc, coeff, eris=None, t1=None, t2=None, t3aaa=None, t3aab=None, t4=None, ci2cc=None, max_cycle=50, tol=1e-8,
           tolnormt=1e-6, verbose=None, thresh=None, stochastic=False, numzero=1e-5):
    log = logger.new_logger(mycc, verbose)
    if eris is None:
        eris = mycc.ao2mo(mycc.mo_coeff)
...
#    idx.get_Q()
    # gen CI coeff
    if not coeff.flagS: coeff.get_S()
    if not coeff.flagD: coeff.get_D()
    if not coeff.flagT: coeff.get_T()
    if thresh is None:
        if not coeff.flagQ: coeff.get_Q()
    else:
        if not coeff.flagQ: coeff.get_Q_thresh(sort_index, coeff.idx)
...
#    if dbg:
    ...
    print ('  0SD (D) =', norm,'(', norm-norm0S,')')
    norm0SD = norm
    norm += numpy.sum(numpy.square ( coeff.T_aaa ))
    norm += numpy.sum(numpy.square ( coeff.T_aab ))
    norm += numpy.sum(numpy.square ( coeff.T_abb ))
    norm += numpy.sum(numpy.square ( coeff.T_bbb ))
    print (' 0SDT (T) =', norm,'(', norm-norm0SD,')')
    norm0SDT = norm
    norm += numpy.sum(numpy.square ( coeff.Q_aaab ))
    norm += numpy.sum(numpy.square ( coeff.Q_aabb ))
    norm += numpy.sum(numpy.square ( coeff.Q_abbb ))
    print ('0SDTQ (Q) =', norm,'(', norm-norm0SDT,')')
...
#    cput1 = log.timer('Gen CI coeff', *cput1)
    ...
    # extract t amplitude
    if not cWrapper: ci2cc = fci_to_cc(nocc, nvir, coeff.idx, coeff.Ref)
    else: ci2cc = fci_to_cc_c(nocc, nvir, coeff.idx, coeff.Ref)
    ...
    if thresh is not None:
        # have to change
        ci2cc.c4_to_t4(coeff.Q_aaab.copy(), coeff.Q_aabb.copy(), numzero=numzero)
        t4_sort = ci2cc.t4.copy().transpose(0,4,1,5,2,3,6,7).reshape(nocc*nvir*nocc*nvir,nocc,nocc,nvir,nvir)
        t4_sort = t4_sort[sort_index,:,:,:,:]
    else:
        ci2cc.c4_to_t4(coeff.Q_aaab.copy(), coeff.Q_aabb.copy(), numzero=numzero)
    ...
    if dbg:
        ci2cc_dbg = fci_to_cc(nocc, nvir, coeff.idx, coeff.Ref)
        ci2cc_dbg.c1_to_t1(coeff.S_a)
#        for i in range(nocc): 
#            for a in range(nvir): 
#                print ('i, a, t1c-t1p', i, a, ci2cc.t1[i][a]-ci2cc_dbg.t1[i][a])
        print('t1: ',numpy.linalg.norm (ci2cc.t1 - ci2cc_dbg.t1 ) )
        ci2cc_dbg.c2_to_t2(coeff.D_aa, coeff.D_ab)  
        print('t2aa: ',numpy.linalg.norm (ci2cc.t2aa - ci2cc_dbg.t2aa ) )
        print('t2ab: ',numpy.linalg.norm (ci2cc.t2ab - ci2cc_dbg.t2ab ) )
#        for i in range(nocc): 
#            for j in range(nocc): 
#                for a in range(nvir): 
#                    for b in range(nvir): 
#                        print ('i, j, a, b, e_aa, e_ab', i, a, ci2cc.t2aa[i][j][a][b]-ci2cc_dbg.t2aa[i][j][a][b], ci2cc.t2ab[i][j][a][b]-ci2cc_dbg.t2ab[i][j][a][b])
        ci2cc_dbg.c3_to_t3(coeff.T_aaa, coeff.T_aab)
        print('t3aaa: ',numpy.linalg.norm (ci2cc.t3aaa - ci2cc_dbg.t3aaa ) )
        print('t3aab: ',numpy.linalg.norm (ci2cc.t3aab - ci2cc_dbg.t3aab ) )
        ci2cc_dbg.c4_to_t4(coeff.Q_aaab.copy(), coeff.Q_aabb.copy())
        print('t4aaab: ',numpy.linalg.norm (ci2cc.t4aaab - ci2cc_dbg.t4aaab ) )
        print('t4aabb: ',numpy.linalg.norm (ci2cc.t4aabb - ci2cc_dbg.t4aabb ) )
#    log.info('max_memory %d MB (current use %d MB after converting t amplitudes)',
#             mycc.max_memory, lib.current_memory()[0])
#    cput1 = log.timer('Gen t amplitude python', *cput1)

    # compute t3 and t4 contribution
    t1 = ci2cc.t1
    t2 = ci2cc.t2ab
    t3aaa = ci2cc.t3aaa
    t3aab = ci2cc.t3aab
    ...
    if dbg:
        norm1 = numpy.sum(numpy.square (t3aaa))
        norm2 = numpy.sum(numpy.square (t3aab))
        print ('t3aaa norm =', norm1)
        print ('t3aab norm =', norm2)

#lsh: T3, T4 contribution
    t1_t3c =0.5*einsum('kcld,iklacd->ia', eris_ovov, t3aaa)
    t1_t3c+=0.5*einsum('kcld,iklacd->ia', eris_ovov, t3aab)
    t1_t3c+=0.5*einsum('kcld,ilkadc->ia', eris_ovov, t3aab)
    t1_t3c+=0.5*einsum('kcld,klicda->ia', eris_ovov, t3aab)

    t3tmp = t3aab.copy() + t3aab.transpose(0,2,1,3,5,4).copy() 
    tmp2  = einsum('kdlc,id->kilc', eris_ovov, t1)
    tmp2 += np.asarray(eris_ooov).conj().copy()
    t2_t3t4c  = -einsum('kilc,lkjcab->ijab', tmp2, t3tmp)
    tmp2  = einsum('kdlc,jd->kjlc', eris_ovov, t1)
    ...
    if dbg:
        norm1 = numpy.sum(numpy.square (t4))
        print ('t4 norm =', norm1)

    if thresh is not None:
        t2_t3t4c +=0.5*einsum('s,sijab->ijab', eris_ovov_sort, t4_sort)
    elif stochastic:
        num_samp=20000
        mean = np.zeros((nocc,nocc,nvir,nvir)) 
        for it in range(num_samp):
            ...
            for i in range(nocc):
                for j in range(nocc):
                    for a in range(nvir):
                        for b in range(nvir):
                            mean[i][j][a][b] += np.sign(eris_ovov[k][c][l][d]) * t4[k][l][i][j][c][d][a][b] 
        mean /= num_samp
        t2_t3t4c +=0.5*mean*cont4
    else:
        t2_t3t4c +=0.5*einsum('kcld,klijcdab->ijab', eris_ovov, t4)
#        p_d_klc = einsum('kcld,kcl->kcld', eris_ovov, cont1)
#        p_c_kl  = einsum('kcl,kl->kcl', cont1, cont2)
#        p_l_k   = einsum('kl,k->kl', cont2, cont3)
#        p_k     = cont3 / cont4 
...
#    t1_t3c = numpy.zeros((nocc,nvir), dtype=numpy.float64)
#    t2_t3t4c = numpy.zeros((nocc,nocc,nvir,nvir), dtype=numpy.float64)

#    mycc.correlation_t4_2e_test(t4, eris)
#    eccsd=0.0
...
#    for istep in range(1):
    for istep in range(max_cycle):
        t1new, t2new = mycc.update_amps(t1, t2, t1_t3c, t2_t3t4c, eris, thresh=thresh)
        tmpvec = mycc.amplitudes_to_vector(t1new, t2new)
        tmpvec -= mycc.amplitudes_to_vector(t1, t2)
        normt = numpy.linalg.norm(tmpvec)
        tmpvec = None
...
http://sunqm.net/pyscf/code-rule.html#api-rules for the details of API conventions''')
        ...
        # For ecCCSD and TCCSD calculation
        self.t4contraction   = True 
        self.onthefly        = True 
        ...
        self.restart = 0  #TODO: old version. need to be updated 
...
# don't modify the following attributes, they are not input options
    ...
    def ccsd(self, t1=None, t2=None, eris=None, ecCCSD=False, ecCCSD_devel=False, TCCSD=False, TCCSD_tmp=False, ecTCCSD=False, ecTCCSD_devel=False, coeff=None, numzero=1E-9):
        ...
        if ecCCSD:
            self.converged, self.e_corr, self.t1, self.t2 = \
                  ec_kernel(self, coeff, eris, t1, t2, max_cycle=self.max_cycle,
                           tol=self.conv_tol, tolnormt=self.conv_tol_normt,
                           verbose=self.verbose, numzero=numzero)
        elif ecCCSD_devel:
            self.converged, self.e_corr, self.t1, self.t2 = \
                  ec_kernel_devel(self, coeff, eris, t1, t2, max_cycle=self.max_cycle,
                           tol=self.conv_tol, tolnormt=self.conv_tol_normt,
                           verbose=self.verbose, numzero=numzero)

        elif TCCSD:
            self.converged, self.e_corr, self.t1, self.t2 = \
                  tcc_kernel(self, coeff, eris, t1, t2, max_cycle=self.max_cycle,
                           tol=self.conv_tol, tolnormt=self.conv_tol_normt,
                           verbose=self.verbose, numzero=numzero)
        elif TCCSD_tmp:
            self.converged, self.e_corr, self.t1, self.t2 = \
                  tcc_kernel_new(self, coeff, eris, t1, t2, max_cycle=self.max_cycle,
                           tol=self.conv_tol, tolnormt=self.conv_tol_normt,
                           verbose=self.verbose, numzero=numzero)
        elif ecTCCSD:
            self.converged, self.e_corr, self.t1, self.t2 = \
    ...
    def ccsd_t(self, t1=None, t2=None, eris=None, ecCCSD=False, ecCCSD_devel=False, coeff=None, TCCSD=False, TCCSD_tmp=False, ecTCCSD=False, ecTCCSD_devel=False):
        if ecCCSD:
            ...
            #lsh test
            #from pyscf.cc import ecccsd_t_slow
            #return ecccsd_t_slow.kernel(self, eris, self.coeff, t1, t2, self.verbose)
        elif ecCCSD_devel:
            from pyscf.cc import ecccsd_t_slow
            if t1 is None: t1 = self.t1
            if t2 is None: t2 = self.t2
            if eris is None: eris = self.ao2mo(self.mo_coeff)
            return ecccsd_t_slow.kernel(self, eris, coeff, t1, t2, self.verbose)
        elif TCCSD:
...
#        if t2 is None: t2 = self.t2
#        if eris is None: eris = self.ao2mo(self.mo_coeff)
#        return renorm_ccsd_t_slow.kernel(self, eris, t1, t2, self.verbose)
